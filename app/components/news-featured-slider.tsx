...
const stories = [
  ...
  { href: "/news/bjby", image: "/figma/news-single/architecture.webp", title: "Beijing Jinxi|Boyuan Real|Estate Co", treatment: "light-image" },
];
...
export function NewsFeaturedSlider() {
  ...
  return <section className="news-featured-slider">
    ...
    <div className="news-featured-track" ref={trackRef} onScroll={syncActive}>
      {stories.map((story) => <Link className={`news-featured-card${story.treatment ? ` news-featured-card--${story.treatment}` : ""}`} href={story.href} key={story.href}><Image src={story.image} alt="" fill sizes="(max-width: 760px) 78vw, 31vw" /><span>{story.title.split("|").map((line, index) => <Fragment key={line}>{index > 0 && <br />}{line}</Fragment>)}</span></Link>)}
    </div>
